    ...
    <template v-else>
        ...
        <div class="scores-block">
          <div class="map-row">
            <span class="map-name-label" v-if="currentMap">{{
              mapDisplayName(currentMap.map_name)
            }}</span>
          ...
          <div class="serie-row" v-if="match.max_maps > 1">
            <span class="serie-label">{{ $t("OverlayLive.Series") }}</span>
      ...
      <div class="player-subheader" v-if="playerStats">
        ...
        <span class="map-number-label" v-if="currentMap">
          . {{ $t("GlobalStats.Map") }} {{ currentMap.map_number }}/{{
            match.max_maps
          }}</span
        >
      </div>
    ...
    async fetchLive() {
      try {
        const res = await this.axioCall.get(
          `${process.env?.VUE_APP_G5V_API_URL || "/api"}/playerstats/${
            this.steamid
          }/live`
        );
        this.applyPayload(res.data);
      } catch (error) {
        // Keep the last known data on a failed poll rather than blanking
        // the overlay for one tick.
      } finally {
        this.loading = false;
      }
    },
...
<style scoped>
/* Root: transparent background for OBS chroma/crop integration. */
.overlay-root {
...
/* Transparent mode: invisible background, dark text. */
